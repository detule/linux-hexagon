...
// Cotulla: compatible layer
struct switch_dev { char *name; void *print_state; void *print_name; u32 state; u32 _dummy; };
#define switch_dev_register(a)		0
#define switch_dev_unregister(a)
#define switch_set_state(a, b)

struct wake_lock { u32 _dummy; };
#define wake_lock(a)
#define wake_unlock(a)
#define wake_lock_init(a, b, c)
#define wake_lock_destroy(a)
...
#define MSM_USB_BASE ((u8*) ui->addr)
...
static irqreturn_t usb_interrupt(int irq, void *data)
{
	...
	printk("usb_interrupt\n");
	n = readl(USB_USBSTS);
	writel(n, USB_USBSTS);
        printk("usb_interrupt %X %d\n", n, atomic_read(&ui->running));
	...
	if (n & STS_UI) {
		printk("usb_interrupt transfer init\n");
...
static int msm72k_probe(struct platform_device *pdev)
{
	...
	ui->xceiv = usb_get_phy(USB_PHY_TYPE_USB2); // usb_get_transceiver();
	if (!ui->xceiv)
