...
static void enable_idgnd(struct msm_otg *dev)
{
	...
	writel(readl(USB_OTGSC) | temp, USB_OTGSC);
}
...
#define msm_otg_send_event(a, b)	0
...
//	dev->set_clk = msm_otg_set_clk;
	...
	ret = usb_add_phy(&dev->phy, USB_PHY_TYPE_USB2);
	if (ret) 
	{
		WARN_ON(1);
		goto free_otg_irq;
